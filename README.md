...
공공 전시 데이터는 서울 열린데이터광장 인증키 사용 시 기본 1,000건까지 동기화한다. `sample` 키는 제공처 제한에 따라 5건으로 제한된다. 종료 후 1개월 이내 전시는 공개 목록의 마지막에 만료 상태로 구분하고 지도에서는 제외한다. 더 오래된 기록은 공개하지 않고 AI의 과거 전시 지식으로만 유지한다.
